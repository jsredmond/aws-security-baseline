...
func TestCloudTrailModuleStructureCompleteness(t *testing.T) {
	t.Parallel()

	modulePath := "../modules/cloudtrail"
	...
	for _, file := range requiredFiles {
		...
		t.Run("File_"+file+"_exists", func(t *testing.T) {
			...
			assert.NoError(t, err, "Required file %s should exist in CloudTrail module", file)
		})
	}
}
