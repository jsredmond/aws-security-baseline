...
def display_banner(console: Console | None = None) -> str:
    ...
    banner_text.append("\n")
    banner_text.append("Deploy AWS security services with confidence\n\n", style="italic white")
...
def display_summary(config: WizardConfig, console: Console | None = None) -> bool:
    ...
    modules_table = Table(title="Security Modules", show_header=True, header_style="bold cyan")
    ...
    for module in AVAILABLE_MODULES:
        ...
        status = "[bold green]✓ Enabled[/bold green]" if enabled else "[dim red]✗ Disabled[/dim red]"
        modules_table.add_row(module.display_name, status, module.description)
    
    # Create settings table
    settings_table = Table(title="Deployment Settings", show_header=True, header_style="bold cyan")
    ...
    # Create tags table
    tags_table = Table(title="Resource Tags", show_header=True, header_style="bold cyan")
...
def display_next_steps(output_path: str, console: Console | None = None) -> str:
    ...
    next_steps_text.append("  Ensure your AWS credentials are configured before running Terraform.\n", style="white")
