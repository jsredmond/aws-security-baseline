...
func TestSecurityHubModuleStructureCompleteness(t *testing.T) {
	t.Parallel()

	modulePath := "../modules/securityhub"
	...
	for _, file := range requiredFiles {
		...
		t.Run("File_"+file+"_exists", func(t *testing.T) {
			...
			assert.NoError(t, err, "Required file %s should exist in Security Hub module", file)
		})
	}
}
