    ...
    **Validates: Requirements 1.1, 1.3**
    ...
    def test_banner_contains_github_url(self):
        ...
        **Validates: Requirements 1.3**
        """
        content = get_banner_content()
        assert GITHUB_URL in content, f"Banner should contain GitHub URL: {GITHUB_URL}"
    ...
    def test_display_banner_returns_content(self):
        ...
        assert GITHUB_URL in result, "Returned content should contain GitHub URL"
    ...
    def test_display_banner_prints_to_console(self):
        ...
        assert (
            "AWS Security Baseline" in printed_output or "Security" in printed_output
...
environment_strategy = st.from_regex(
    r"[a-zA-Z0-9][a-zA-Z0-9-]*", fullmatch=True
...
tags_strategy = st.dictionaries(
    keys=tag_key_strategy, values=tag_value_strategy, min_size=0, max_size=5
)
    ...
    def test_summary_contains_all_tags(self, config: WizardConfig):
        ...
        **Validates: Requirements 6.1, 6.2**
        ...
        for key, value in config.tags.items():
            assert key in summary, f"Summary should contain tag key '{key}'"
            assert value in summary, f"Summary should contain tag value '{value}'"
    ...
    **Validates: Requirements 8.1, 8.2, 8.3**
    ...
    def test_display_next_steps_without_console_argument(self):
        ...
        assert result is not None, (
            "display_next_steps should work without console argument"
        )
